...
# Pushing the v<version> tag triggers .github/workflows/publish.yml, which builds
# the UI + CLI and runs `npm publish --access public`.
#
...
version=$(npm version "$BUMP" --no-git-tag-version)   # e.g. v0.1.1
...
echo "Pushed $version. CI will publish to npm: https://github.com/quangtd-kozocom/bull-terra/actions"
